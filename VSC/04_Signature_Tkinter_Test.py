...
#######################################
#activate it if you want to invalid the blockchain and amend the amount
...
# Assuming the necessary classes (Transaction, Blockchain) are defined as in the previous code
...
class BlockchainApp:
    def __init__(self, root):
        ...
        self.nik_chain = Blockchain()  # Your Blockchain object

        # Public and Private Key Entry
        self.public_key_label = tk.Label(root, text="Public Key:")
        self.public_key_label.grid(row=0, column=0)
        self.public_key_entry = tk.Entry(root)
        self.public_key_entry.grid(row=0, column=1)

        self.private_key_label = tk.Label(root, text="Private Key:")
        self.private_key_label.grid(row=1, column=0)
        self.private_key_entry = tk.Entry(root)
        self.private_key_entry.grid(row=1, column=1)

        # Transaction details
        self.from_address_label = tk.Label(root, text="From Address:")
        self.from_address_label.grid(row=2, column=0)
        self.from_address_entry = tk.Entry(root)
        self.from_address_entry.grid(row=2, column=1)

        self.to_address_label = tk.Label(root, text="To Address:")
        self.to_address_label.grid(row=3, column=0)
        self.to_address_entry = tk.Entry(root)
        self.to_address_entry.grid(row=3, column=1)
        ...
        self.add_transaction_button.grid(row=5, column=0, columnspan=2)

        self.mine_button = tk.Button(root, text="Mine Pending Transactions", command=self.mine_pending_transactions)
        self.mine_button.grid(row=6, column=0, columnspan=2)
    ...
    def add_transaction(self):
        try:
            ...
            self.nik_chain.add_transaction(transaction)
    ...
    def mine_pending_transactions(self):
        try:
            # Get the public key (for mining reward address)
            mining_address = self.public_key_entry.get()

            # Mine the pending transactions
            self.nik_chain.mine_pending_transactions(mining_address)

            messagebox.showinfo("Success", "Block mined successfully!")
        except Exception as e:
            messagebox.showerror("Error", f"Error mining transactions: {str(e)}")
